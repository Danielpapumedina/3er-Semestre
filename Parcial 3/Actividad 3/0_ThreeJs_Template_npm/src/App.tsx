...
world.gravity = new CANNON.Vec3(0,-9.81,0);
  ...
  const planoPhysM = new CANNON.Material("reboton");
  ...
  cuboMesh.position.set(0,15,0);


  const cuboPhysM = new CANNON.Material("cuboMathPhys");

 const cuboBody: any = new CANNON.Body({
  mass:1,
 ...
 const pisoCuboContact = new CANNON.ContactMaterial(planoPhysM,
  cuboPhysM,{
    friction:1,
    //restitution:1
  })
  ...
  // esferasM.push(esfeMesh);
  // esferasB.push(esferaBody);

  const spawnesferas = setInterval(()=>{
  const esferaGeo = new THREE.SphereGeometry(2);
  const esferaMat = new THREE.MeshPhongMaterial({
    color: 0xeebb77,
    wireframe: false
  ...
  cantidadParticles = cantidadParticles+1;
  if(cantidadParticles==llamado){
    clearInterval(spawnesferas);
  }
  },100);
  ...
  function animate() {
    ...
    for (let i = 0; i< esferasM. length; i++){
      const M = esferasM[i];
      const B = esferasB[i];
