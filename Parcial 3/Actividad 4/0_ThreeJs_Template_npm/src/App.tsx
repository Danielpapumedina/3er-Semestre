...
function doThreeJS(){
  ...
  let muebles = ['Habitacion','Sillon', 'Banquito', 'Almohada', 'Contoller','Mesa','Agave','Book','Cactus','TV', 'Wireframe','lamp','Alfombra', 'icosphere', 'cubotorcido', 'piso' ]
  ...
  // p.textContent = p1;//ponemos texto
  
  const pCSSObject = new CSS2DObject(div);
  scene.add(pCSSObject); //funciona como un Billboard
  p.className = 'tooltip show'
  pCSSObject.position.set(6,2,0);
  ...
  window.addEventListener('click',(e)=>{
      ...
      if(intersects.length>0){
        ...
        if(currentTimeout!=undefined){clearTimeout(currentTimeout);
          ...
          const result = muebles.filter((word) => word !== "Sillon");
          console.log(result);
...
gltloader.load('/HouseModel/coolHouse.gltf',
function(gltf){
  modelo = gltf.scene;
  scene.add(modelo);
